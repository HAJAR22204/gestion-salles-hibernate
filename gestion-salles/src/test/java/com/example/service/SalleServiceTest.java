...
import org.junit.After; //nettoyage
import org.junit.Before; //preparation
...
public class SalleServiceTest {
    ...
    public void testFindByDisponible() {
        // Create test rooms
        Salle s1 = new Salle("Salle disponible", 20);
        s1.setDisponible(true);

        Salle s2 = new Salle("Salle indisponible", 30);
        ...
        assertTrue(availableRooms.stream().anyMatch(s -> s.getNom().equals("Salle disponible")));
        assertFalse(availableRooms.stream().anyMatch(s -> s.getNom().equals("Salle indisponible")));

        List<Salle> unavailableRooms = service.findByDisponible(false);
        assertTrue(unavailableRooms.stream().anyMatch(s -> s.getNom().equals("Salle indisponible")));
    ...
    public void testFindByCapaciteMinimum() {
        // Create test rooms
        Salle s1 = new Salle("Petite salle", 10);
        Salle s2 = new Salle("Salle moyenne", 50);
        Salle s3 = new Salle("Grande salle", 100);
        ...
        assertEquals(2, roomsMin50.stream()
                .filter(s -> s.getNom().equals("Salle moyenne") || s.getNom().equals("Grande salle"))
                .count());

        List<Salle> roomsMin80 = service.findByCapaciteMinimum(80);
        assertEquals(1, roomsMin80.stream()
                .filter(s -> s.getNom().equals("Grande salle"))
                .count());
